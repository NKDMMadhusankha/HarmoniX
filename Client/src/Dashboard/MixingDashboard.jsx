...
const MusicianProfileEditInline = () => {
  ...
  // State for mixing engineers data
  const [mixingEngineers, setMixingEngineers] = useState([]);
  const [loadingMixingEngineers, setLoadingMixingEngineers] = useState(true);
  const [mixingEngineersError, setMixingEngineersError] = useState(null);

  useEffect(() => {
    const fetchMixingEngineers = async () => {
      setLoadingMixingEngineers(true);
      try {
        const response = await fetch('http://localhost:5000/api/musician/mixing-engineers');
        const data = await response.json();
        if (data.success) {
          setMixingEngineers(data.engineers);
        } else {
          setMixingEngineersError(data.message || 'Failed to fetch mixing engineers');
        }
      } catch (err) {
        setMixingEngineersError('Server error');
      } finally {
        setLoadingMixingEngineers(false);
      }
    };

    fetchMixingEngineers();
  }, []);
