...
const MusicianProfileEditInline = () => {
  ...
  // State for mastering engineers data
  const [masteringEngineers, setMasteringEngineers] = useState([]);
  const [loadingMasteringEngineers, setLoadingMasteringEngineers] = useState(true);
  const [masteringEngineersError, setMasteringEngineersError] = useState(null);

  useEffect(() => {
    const fetchMasteringEngineers = async () => {
      setLoadingMasteringEngineers(true);
      try {
        const response = await fetch('http://localhost:5000/api/musician/mastering-engineers');
        const data = await response.json();
        if (data.success) {
          setMasteringEngineers(data.engineers);
        } else {
          setMasteringEngineersError(data.message || 'Failed to fetch mastering engineers');
        }
      } catch (err) {
        setMasteringEngineersError('Server error');
      } finally {
        setLoadingMasteringEngineers(false);
      }
    };
    fetchMasteringEngineers();
  }, []);
