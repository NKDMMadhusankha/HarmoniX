...
const MixingEngineersPage = () => {
  ...
  useEffect(() => {
    ...
    // Fetch mixing engineers from backend
    axios
      .get('http://localhost:5000/api/musician/mixing-engineers')
      .then((res) => {
        if (res.data.success) {
          setMixingEngineers(res.data.engineers);
        }
        setLoading(false);
      })
      .catch((err) => {
        console.error('Error fetching mixing engineers:', err);
        setLoading(false);
      });
  }, []);

  const handleSeeMore = (engineerInfo) => {
    window.location.href = `/music/mixing-engineer/${engineerInfo.id}`;
  };
