...
const MusicProducerProfile = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/musician/producers/${id}`)
      .then(res => res.json())
      .then(data => {
        if (data && data.success && data.producer) {
          setProfile(data.producer);
          setTracks(data.producer.featuredTracks || []);
          setGalleryImages(data.producer.galleryImages || []);
          setGenres(data.producer.genres || []);
          setSkills(data.producer.skills || []);
          setTools(data.producer.tools || []);
          
          // Handle links - combine both links array and socialMedia/portfolioLinks
          const combinedLinks = [
            ...(data.producer.links || []),
            ...(data.producer.socialMedia?.instagram ? [{
              platform: 'Instagram',
              url: `https://instagram.com/${data.producer.socialMedia.instagram}`,
              type: 'instagram'
            }] : []),
            ...(data.producer.socialMedia?.twitter ? [{
              platform: 'Twitter',
              url: `https://twitter.com/${data.producer.socialMedia.twitter}`,
              type: 'twitter'
            }] : []),
            ...(data.producer.portfolioLinks?.spotify ? [{
              platform: 'Spotify',
              url: data.producer.portfolioLinks.spotify,
              type: 'spotify'
            }] : []),
            ...(data.producer.portfolioLinks?.youtube ? [{
              platform: 'YouTube',
              url: data.producer.portfolioLinks.youtube,
              type: 'youtube'
            }] : []),
            // Add other social/portfolio links as needed
          ];
          
          setLinks(combinedLinks);
          
          setCoverImage(data.producer.coverImage || '');
          setAvatar(data.producer.profileImage || '');
        }
      });
  }, [id]);
  ...
  const handleSubmitContact = async (e) => {
    ...
    // Client-side validation (optional, since you use 'required' in inputs)
    ...
    // Ensure profile and ID are present
    if (!profile || !(profile._id || profile.id)) {
      setSnackbarMessage('Profile not loaded. Please try again.');
      setSnackbarSeverity('error');
      setSnackbarOpen(true);
      return;
    }
    ...
    try {
      const response = await fetch('http://localhost:5000/api/musician/contact', {
        ...
        body: JSON.stringify({
          musicianId: profile._id || profile.id,
