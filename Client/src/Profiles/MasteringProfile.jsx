...
const MusicProducerProfile = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`http://localhost:5000/api/musician/mastering-engineers/${id}`)
      .then(res => {
        if (!res.ok) throw new Error('Mastering Engineer not found');
        return res.json();
      })
      .then(data => {
        if (data.success && data.engineer) {
          ...
          setCoverImage(data.engineer.coverImage || ''); // Ensure cover image is set
          setAvatar(data.engineer.profileImage || ''); // Ensure profile image is set
        } else {
          setError('No engineer data found');
        }
        setLoading(false);
      })
      .catch(err => {
